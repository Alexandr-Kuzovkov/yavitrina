function main(splash, args)
  splash.resource_timeout = 60.0
    ...
    print(cookies)
    values2.mot = search
    assert(form2:fill(values2))
    form2:submit()
    assert(splash:wait(15.0))
    local links = {}
    local elements = splash:select_all('a[class="fancybox openCV"]')
    for i=1, #elements do
      links[i] = elements[i].attributes['lancement']
      print('Link added: '..links[i])
    end
    --print(links)
    --data['png'] = splash:png()
    --data['links'] = links
