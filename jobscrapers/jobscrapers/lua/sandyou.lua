...
function parse_jobs(splash, data)
    local links = splash:select_all('h3.node__title > a')
    local titles = splash:select_all('h3.node__title > a > span')
    local j
  	for j in ipairs(links) do
        data[#data+1] = {link = links[j].attributes['href'], title = titles[j]:text()}
    end
end
...
function count_jobs(splash)
	  local items = splash:select_all('h3.node__title')
    --print(#items)
    return #items
end
