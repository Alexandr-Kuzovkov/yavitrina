...
###############################################
## Copy all data from one database to other
###############################################
import psycopg2
import logging
...
class Datacopy():
    ...
    def __init__(self, src=None, dest=None):
        ...
        self.logger.info('RUN datacopy')

    def delete(self):
        self.logger.info('...run deleting...')
        tables = self.src._get_tables_list()
        while len(tables) > 0:
            table = tables[0]
            ...
            try:
                ...
                self.logger.info('...Table %s cleaned' % table)
            except psycopg2.Error, ex:
                tables.append(tables.pop(0))
                self.logger.info('...%s, cancel, switch to next table' % ex.pgerror)
            else:
                tables.pop(0)
        self.logger.info('Deleting done')

    def copy(self, max_rows=None):
        ...
        while len(tables) > 0:
            ...
            if table in self.exclude_tables:
                tables.pop(0)
                continue
            count = self.src._count_rows(table)
            if count == 0:
                tables.pop(0)
                continue
            ...
            try:
                for offset in offsets:
                    ...
                    data = self.src._get(table, field_list=None, tail='ORDER BY id OFFSET %s LIMIT %s', data=[offset, self.MAX_ROWS])
                ...
                tables.append(tables.pop(0))
                self.logger.info('...cancel, switch to next table')
            else:
                tables.pop(0)
                if table not in self.exclude_seq:
